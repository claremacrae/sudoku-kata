...
namespace SudokuKata
{
	...
	//			#region Construct fully populated board
		...
		std::vector<std::vector<wchar_t>> board = {std::vector<wchar_t>(line.begin(), line.end()), std::vector<wchar_t>(middle.begin(), middle.end()), std::vector<wchar_t>(middle.begin(), middle.end()), std::vector<wchar_t>(middle.begin(), middle.end()), std::vector<wchar_t>(line.begin(), line.end()), std::vector<wchar_t>(middle.begin(), middle.end()), std::vector<wchar_t>(middle.begin(), middle.end()), std::vector<wchar_t>(middle.begin(), middle.end()), std::vector<wchar_t>(line.begin(), line.end()), std::vector<wchar_t>(middle.begin(), middle.end()), std::vector<wchar_t>(middle.begin(), middle.end()), std::vector<wchar_t>(middle.begin(), middle.end()), std::vector<wchar_t>(line.begin(), line.end())};
			...
			if (command == L"expand")
			{
				...
				for (int index = 0; index < currentState.size(); index++)
				{
					if (currentState[index] == 0)
					{
						...
						if (bestCandidatesCount < 0 || candidatesCount < bestCandidatesCount || (candidatesCount == bestCandidatesCount && randomValue < bestRandomValue))
						{
							bestRow = row;
							bestCol = col;
...
//ORIGINAL LINE: int[,] removedPerBlock = new int[3, 3];
		std::vector<std::vector<int>> removedPerBlock = RectangularVectors::RectangularIntVector(3, 3);
	...
	//				#region Build a collection (named cellGroups) which maps cell indices into distinct groups (rows/columns/blocks)
			auto rowsIndices = state.Select([&] (value, index)
			{
				new {Discriminator = index / 9, Description = StringHelper::formatSimple(L"row #{0}", index / 9 + 1), Index = index, Row = index / 9, Column = index % 9};
			})->GroupBy([&] (std::any tuple)
			{
				tuple::Discriminator;
			});

			auto columnIndices = state.Select([&] (value, index)
			{
				new {Discriminator = 9 + index % 9, Description = StringHelper::formatSimple(L"column #{0}", index % 9 + 1), Index = index, Row = index / 9, Column = index % 9};
			})->GroupBy([&] (std::any tuple)
			{
				tuple::Discriminator;
			});

			auto blockIndices = state.Select([&] (value, index)
			{
				new {Row = index / 9, Column = index % 9, Index = index};
			})->Select([&] (std::any tuple)
			{
				new {Discriminator = 18 + 3 * (tuple::Row / 3) + tuple::Column / 3, Description = StringHelper::formatSimple(L"block ({0}, {1})", tuple::Row / 3 + 1, tuple::Column / 3 + 1), Index = tuple::Index, Row = tuple::Row, Column = tuple::Column};
			})->GroupBy([&] (std::any tuple)
			{
				tuple::Discriminator;
			});
	...
	//					#region Pick cells with only one candidate left
				...
				std::vector<int> singleCandidateIndices = candidateMasks.Select([&] (mask, index)
				{
					new {CandidatesCount = maskToOnesCount[mask], Index = index};
				})->Where([&] (std::any tuple)
				{
					return tuple->CandidatesCount == 1;
				})->Select([&] (std::any tuple)
				{
					return tuple::Index;
				})->ToArray();
				...
				if (!changeMade)
				{
					...
					for (int digit = 1; digit <= 9; digit++)
					{
						...
						for (int cellGroup = 0; cellGroup < 9; cellGroup++)
						{
							...
							if (rowNumberCount == 1)
							{
								groupDescriptions.push_back(StringHelper::formatSimple(L"Row #{0}", cellGroup + 1));
							...
							if (colNumberCount == 1)
							{
								groupDescriptions.push_back(StringHelper::formatSimple(L"Column #{0}", cellGroup + 1));
							...
							if (blockNumberCount == 1)
							{
								...
								groupDescriptions.push_back(StringHelper::formatSimple(L"Block ({0}, {1})", blockRow + 1, blockCol + 1));
					...
					if (candidates.size() > 0)
					{
						...
						std::wstring message = std::wstring::Format(L"{0} can contain {1} only at ({2}, {3}).", description, digit, row + 1, col + 1);
	...
	//					#region Try to find pairs of digits in the same row/column/block and remove them from other colliding cells
				if (!changeMade)
				{
					std::vector<int> twoDigitMasks = candidateMasks.Where([&] (std::any mask)
					{
						return maskToOnesCount[mask] == 2;
					})->Distinct()->ToList();

					auto groups = twoDigitMasks.SelectMany([&] (std::any mask)
					{
						cellGroups.Where([&] (std::any group)
						{
							return group->Count([&] (std::any tuple)
							{
							return candidateMasks[tuple::Index] == mask;
							}) == 2;
						})->Where([&] (std::any group)
						{
							group::Any([&] (std::any tuple)
							{
							return candidateMasks[tuple::Index] != mask && (candidateMasks[tuple::Index] & mask) > 0;
							});
						})->Select([&] (std::any group)
						{
							new {Mask = mask, Discriminator = group::Key, Description = group::First()->Description, Cells = group};
						});
					})->ToList();
					...
					if (groups.Any())
					{
						...
						{
							auto cells = group.Cells::Where([&] (std::any cell)
							{
								return candidateMasks[cell::Index] != group.Mask && (candidateMasks[cell::Index] & group.Mask) > 0;
							})->ToList();

							auto maskCells = group.Cells::Where([&] (std::any cell)
							{
	...
	//					#region Try to find groups of digits of size N which only appear in N cells within row/column/block
				...
				{
					std::vector<int> masks = maskToOnesCount.Where([&] (std::any tuple)
					{
					...
					auto groupsWithNMasks = masks.SelectMany([&] (std::any mask)
					{
						cellGroups.Where([&] (std::any group)
						{
							group::All([&] (std::any cell)
							{
							return state[cell::Index] == 0 || (mask & (1 << (state[cell::Index] - 1))) == 0;
							});
						})->Select([&] (std::any group)
						{
							new { Mask = mask, Description = group::First()->Description, Cells = group, CellsWithMask = group::Where([&] (std::any cell)
							{
							return state[cell::Index] == 0 && (candidateMasks[cell::Index] & mask) != 0;
							})->ToList(), CleanableCellsCount = group->Count([&] (std::any cell)
							{
							return state[cell::Index] == 0 && (candidateMasks[cell::Index] & mask) != 0 && (candidateMasks[cell::Index] & ~mask) != 0;
						}) };
						});
					})->Where([&] (std::any group)
					{
						return group::CellsWithMask->Count() == maskToOnesCount[group::Mask];
					})->ToList();
					...
					for (auto groupWithNMasks : groupsWithNMasks)
					{
						...
						if (groupWithNMasks.Cells::Any([&] (std::any cell)
						{
							return (candidateMasks[cell::Index] & mask) != 0 && (candidateMasks[cell::Index] & ~mask) != 0;
						}))
						{
							StringBuilder *message = new StringBuilder();
							message->append(StringHelper::formatSimple(L"In {0} values ", groupWithNMasks.Description));
							...
							while (temp > 0)
							{
								if ((temp & 1) > 0)
								{
									message->append(StringHelper::formatSimple(L"{0}{1}", separator, curValue));
									separator = L", ";
								}
								temp = temp >> 1;
								curValue += 1;
							...
							{
								message->append(StringHelper::formatSimple(L" ({0}, {1})", cell->Row + 1, cell->Column + 1));
							}
						...
						for (auto cell : groupWithNMasks.CellsWithMask)
						{
							...
							while (maskToClear > 0)
							{
								if ((maskToClear & 1) > 0)
								{
									message->append(StringHelper::formatSimple(L"{0}{1}", separator, valueToClear));
									separator = L", ";
								}
	...
	//				#region Final attempt - look if the board has multiple solutions
			if (!changeMade)
			{
				...
				while (candidateIndex1.Any())
				{
					...
					while (command != L"complete" && command != L"fail")
					{
						if (command == L"expand")
						{
							...
							{
								std::copy_n(alternateState.begin(), currentState.size(), currentState.begin());
							}
						...
						else if (command == L"move")
						{
							...
							if (movedToDigit <= 9)
							{
								...
								if (currentState.Any([&] (std::any digit)
								{
									return digit == 0;
								}))
								{
				...
				if (stateIndex1.Any())
				{
					...
					{
						description = StringHelper::formatSimple(L"block ({0}, {1})", row1 / 3 + 1, col1 / 3 + 1);
					}
	...
	//					#region Print the board as it looks after one change was made to it
				...
				std::wstring code = StringHelper::replace(StringHelper::replace(StringHelper::replace(StringHelper::replace(std::wstring::Join(L"", board.Select([&] (std::any s)
				{
					return std::wstring(s);
